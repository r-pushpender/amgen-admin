...
const createLogStream = (
  logStreamName,
  { logGroupName, region, accessKeyId, secretAccessKey, local }
) => {
  if (local && local != 'false')
    return Promise.resolve({
      message: 'No need to create stream. Local mode is on.',
    });
  ...
  if (!secretAccessKey)
    throw new Error(
      "Missing required argument: 'secretAccessKey' is required."
    );
  if (!accessKeyId)
  ...
  const { uri, headers } = getRequestParams(service, region, payload, {
    accessKeyId,
    secretAccessKey,
  });
...
const addLogsToStream = (
  ...
  const events =
    // entry is a message
    entryType == 'string'
      ? [{ message: entry, timestamp: now }]
      : // entry is a well-formatted log event
      entryType == 'object' && entry.timestamp
  ...
  return retryCount > 3 || nothingToLog
...
const delayFn = (fn, time) =>
  makeQuerablePromise(
    new Promise((onSuccess) => setTimeout(() => onSuccess(), time)).then(() =>
      fn()
    )
  );
...
const Logger = class {
  constructor({
    ...
    if (!logStreamName)
      throw new Error(
        "Missing required argument: 'logStreamName' is required."
      );
    if (!region)
      throw new Error("Missing required argument: 'region' is required.");
    if (!secretAccessKey)
    ...
    if (!uploadFreq || uploadFreq < 0) {
      ...
      log = (...args) => {
        ...
        let latestBuffer = _logbuffer.get(this) || {
          latest: now,
          data: [],
          job: null,
        };
        ...
        if (!latestBuffer.job || !latestBuffer.job.isPending()) {
          latestBuffer.job = makeQuerablePromise(
            delayFn(() => {
              const { latest, data, job } = _logbuffer.get(this) || {
                latest: now,
                data: [],
                job: null,
              };
